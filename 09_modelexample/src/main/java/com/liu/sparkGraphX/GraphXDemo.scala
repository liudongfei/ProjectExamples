...
object GraphXDemo{
  def main(args: Array[String]): Unit = {
    ...
    graph.edges.filter{e => e.attr >5}.collect().foreach{
      e => println(s"${e.srcId} to ${e.dstId} is ${e.attr}")
    }

    //triplets操作，((srcId, srcAttr), (dstId, dstAttr), attr)
    println("列出边属性>5的tripltes：")
    graph.triplets.filter{t => t.attr > 5}.collect().foreach{
      t => println(s"${t.srcAttr._1} like ${t.dstAttr._1}")
    }
    ...
    println("maxInDegrees:" + graph.inDegrees.reduce(max))
    println("maxOutDegrees:" + graph.outDegrees.reduce(max))
    println("maxDegrees:" + graph.degrees.reduce(max))
    //***********************************************************************************
    //***************************  转换操作    ****************************************
    //**********************************************************************************
    println("**********************************************************")
    ...
    graph.mapEdges(e => e.attr * 2).edges.collect().foreach{
      e => println(s"${e.srcId} to ${e.dstId} is ${e.attr}")
    }
    //***********************************************************************************
    //***************************  结构操作    ****************************************
    ...
    println("所有子图的边：")
    subGraph.edges.collect().foreach(e => println(s"${e.srcId} to ${e.dstId} : ${e.attr}"))
    //***********************************************************************************
    //***************************  连接操作    ****************************************
    //**********************************************************************************
    println("**********************************************************")
    ...
    initialUserGraph.vertices.foreach(u => println(u._1 + ":" + u._2.name + "is" + u._2.age))
    initialUserGraph.edges.foreach(e => println(e.srcId + "to" + e.dstId + ":" + e.attr))
    ...
    println("**********************************************************")
    println("找出年纪最大的追求者：")
    //mapReduceTriplets被替换成aggregateMessages
    // 得到最大追求者，reduce过程
