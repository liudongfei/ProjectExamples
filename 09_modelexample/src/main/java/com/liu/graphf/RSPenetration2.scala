...
object RSPenetration2 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val danbaodf1 = spark.createDataFrame(List(
      ...
      ("7","七", "8", "八", "担保", "a"),
      ("9","九", "10","十", "担保", "a")
    )).toDF("cust_no","cust_name", "guar_no","guar_name", "rs", "sa")
    //对外关系表
...
//    spark.udf.register("stringPlus", new StringPlus)
    ...
    //    val tmpDF1 = circleDF.groupBy("src_dst").agg(min("length").as("length"))
    ////      .withColumnRenamed("min(length)", "length")
    //      .join(circleDF, Seq("src_dst", "length"), "inner")
    ////    //对担保关系链表的总表添加序号列
    //    val w = Window.orderBy("src_dst")
    //    val tmpDF2 = tmpDF1.withColumn("circleIndex", row_number().over(w)).show()
    //
    //    /**
    //      * 对担保关系链拆分成边的形式，
    ...
    //    spark.createDataFrame(rdd2, structType2).withColumn("model_date", current_date())
    //      .show()
    //
    ////      .repartition(1)
