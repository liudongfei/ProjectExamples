 ...
 * @Description:
 */
public class NettyNioHttpsClientFactory {
    private static final Logger logger = LoggerFactory.getLogger(NettyNioHttpsClientFactory.class);
    ...
    private static NettyNioHttpsClientFactory instance;
    /**
     * 工厂类的初始化.
    ...
    public static NettyNioHttpsClientFactory buildFactory(ChannelInboundHandler handler,
                                                          Integer maxPerPoolSize, Integer threadNum, Integer maxMessageLen) {
        synchronized (NettyNioHttpsClientFactory.class) {
            if (instance == null) {
                NettyNioHttpsClientFactory factory = new NettyNioHttpsClientFactory();
    ...
    private void init(ChannelInboundHandler handler, Integer maxPerPoolSize,
        ...
        ChannelPoolHandler poolHandler =
                new NettyNioHttpsClientFactory.NettyHttpChannelPoolHandler(handler, maxMessageLen);
    ...
    public static void main(String[] args) throws URISyntaxException {
        ...
        NettyNioHttpsClientFactory.buildFactory(handler, null, null, 1024);
        ...
        for (int i = 0; i < 10; i++) {
            ...
            Future<Channel> f = NettyNioHttpsClientFactory.getChannel(key);
            f.addListener((FutureListener<Channel>) f1 -> {
                if (f1.isSuccess()) {
                    Channel ch = f1.getNow();
                    ch.writeAndFlush(request);
                    // Release back to pool
                    NettyNioHttpsClientFactory.releaseChannel(key, ch);
                }
            });
