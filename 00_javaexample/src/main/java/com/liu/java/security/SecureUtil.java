...
public class SecureUtil {
    ...
    public static String encryptData(String dataString, String encoding, PublicKey key) {
        ...
        try {
            ...
            res = new String(SecureUtil.base64Encode(data), encoding);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return res;
    ...
    private static byte[] encryptData(PublicKey publicKey, byte[] plainData) {
        byte[] res = null;
        try {
            // RSA/ECB/PKCS1Padding -> "算法/工作模式/填充模式"
            Cipher cipher = Cipher.getInstance("RSA/ECB/PKCS1Padding","BC");
            cipher.init(Cipher.ENCRYPT_MODE, publicKey);
            res = cipher.doFinal(plainData);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return res;
